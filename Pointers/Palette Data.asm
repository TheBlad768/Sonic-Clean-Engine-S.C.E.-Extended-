; ===========================================================================
; Normal palette
; ===========================================================================

Pal_Sonic:			binclude "Objects/Sonic/Palettes/Sonic.bin"
	even
Pal_WaterSonic:		binclude "Objects/Sonic/Palettes/Water Sonic.bin"
	even
Pal_Knuckles:		binclude "Objects/Knuckles/Palettes/Knuckles.bin"
	even
Pal_WaterKnuckles:	binclude "Objects/Knuckles/Palettes/Water Knuckles.bin"
	even
Pal_LevelSelect:		binclude "Data/Screens/Level Select/Palettes/1.bin"
	even
Pal_Continue:		binclude "Data/Screens/Continue/Palettes/1.bin"
	even
Pal_DEZ:			binclude "Levels/DEZ/Palettes/Death Egg Zone.bin"
	even
Pal_WaterDEZ:		binclude "Levels/DEZ/Palettes/Water Death Egg Zone.bin"
	even

; ===========================================================================
; Dynamic palette
; ===========================================================================

AnPal_PalDEZ12_1:	binclude "Levels/DEZ/Palettes/Animate/1.bin"
	even
AnPal_PalDEZ12_2:	binclude "Levels/DEZ/Palettes/Animate/2.bin"
	even